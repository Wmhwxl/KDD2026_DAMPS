...
class LIRDRec(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if self.v_feat is not None:
            self.v_feat, self.t_feat = self.feature_filter(self.v_feat, self.t_feat)
            ...
            self.v_preference = nn.Parameter(nn.init.xavier_normal_(torch.tensor(
                ...
                self.device))
            self.v_MLP = nn.Linear(v_feat_dim, self.embedding_dim)
            self.v_MLP_1 = nn.Linear(self.embedding_dim, self.embedding_dim, bias=False)
        if self.t_feat is not None:
            ...
            self.t_preference = nn.Parameter(nn.init.xavier_normal_(torch.tensor(
                np.random.randn(self.n_users, self.embedding_dim), dtype=torch.float32, requires_grad=True), gain=1).to(
                self.device))
            self.t_MLP = nn.Linear(t_feat_dim, self.embedding_dim)
            self.t_MLP_1 = nn.Linear(self.embedding_dim, self.embedding_dim, bias=False)

        self.id_preference = nn.Parameter(nn.init.xavier_normal_(torch.tensor(
            np.random.randn(self.n_users, self.embedding_dim), dtype=torch.float32, requires_grad=True), gain=1).to(
            self.device))
        self.s_MLP = nn.Linear(t_feat_dim + v_feat_dim, self.embedding_dim)
        self.s_MLP_1 = nn.Linear(self.embedding_dim, self.embedding_dim, bias=False)
    ...
    def forward(self, adj):
        ...
        tmp_v_feat, tmp_t_feat = self.feature_filter(tmp_v_feat, tmp_t_feat)
