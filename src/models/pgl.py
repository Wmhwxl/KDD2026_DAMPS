...
class PGL(GeneralRecommender):
    ...
    def global_subgraph_extraction(self, adj):
        norm_adj = adj.tocsc()
        # 使用 scipy.sparse.linalg.svds 替代 sparsesvd
        ut, s, vt = slinalg.svds(norm_adj, k=self.embedding_dim)
        
        # 注意：svds 返回的奇异值是升序排列的，需要翻转
        ut = ut[:, ::-1]
        s = s[::-1]
        vt = vt[::-1]
        ...
        product_sparse_matrix = ut[:, :num_top_bottom] @ product_matrix @ vt[:num_top_bottom, :]
